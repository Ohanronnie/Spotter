...
export const searchFlights = async (originSkyId, destinationSkyId, originEntityId, destinationEntityId, date, returnDate, cabinClass, childQuery) => {
  ...
  const response = await fetch(
    `${url}?originSkyId=${originSkyId}&destinationSkyId=${destinationSkyId}&originEntityId=${originEntityId}&destinationEntityId=${destinationEntityId}&date=${date}${returnDate ? '&returnDate='+returnDate : ''}&cabinClass=${cabinClass}${childQuery}`,
    options
  );
  const data = await response.json();
  return data
...
export const getFlightDetails = async (origin, destination, date,returnDate, cabinClass, childQuery) => {
